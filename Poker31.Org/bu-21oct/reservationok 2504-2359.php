...
if ( (isset($_POST['submit']) && ($prenom<>"-Anonyme-")) or ((isset($_POST['submit'])&&(!empty($co))))   )
 ...
 if(isset($_POST['bmodif'])&&!empty($prenom))
 {
    ...
    //   $exe=mysqli_query($conn,"update passager set prenom='$prenom',choix_class='$ch_classe' where prenom='$prenom' ");
    if($exe){
    echo"Modification réussie !!"; 
    header("Location: index.php");
    }
...
$exe=mysqli_query($conn,$sql);
	print'<table border="7" class="tab"><tr><th>Ordre d inscription</th><th>Prénom</th><th>Type d inscription</th></tr>';
